...
type ApiValidateCheckoutAttemptRequest struct {
	ctx context.Context
	ApiService *CheckoutAPIService
	checkoutAttemptId string
}

func (r ApiValidateCheckoutAttemptRequest) Execute() (*CheckoutSuccessResponse, *http.Response, error) {
	return r.ApiService.ValidateCheckoutAttemptExecute(r)
}

/*
ValidateCheckoutAttempt Validate Checkout Attempt

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param checkoutAttemptId The checkout attempt ID
 @return ApiValidateCheckoutAttemptRequest
*/
func (a *CheckoutAPIService) ValidateCheckoutAttempt(ctx context.Context, checkoutAttemptId string) ApiValidateCheckoutAttemptRequest {
	return ApiValidateCheckoutAttemptRequest{
		ApiService: a,
		ctx: ctx,
...
func (a *CheckoutAPIService) ValidateCheckoutAttemptExecute(r ApiValidateCheckoutAttemptRequest) (*CheckoutSuccessResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *CheckoutSuccessResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CheckoutAPIService.ValidateCheckoutAttempt")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/checkout/checkout_attempts/validate/{checkout_attempt_id}"
	localVarPath = strings.Replace(localVarPath, "{"+"checkout_attempt_id"+"}", url.PathEscape(parameterValueToString(r.checkoutAttemptId, "checkoutAttemptId")), -1)
...
type ApiVerifyCheckoutSessionPaymentMethodRequest struct {
	ctx context.Context
	ApiService *CheckoutAPIService
...
func (r ApiVerifyCheckoutSessionPaymentMethodRequest) Execute() (*CheckoutSessionExternal, *http.Response, error) {
	return r.ApiService.VerifyCheckoutSessionPaymentMethodExecute(r)
}

/*
VerifyCheckoutSessionPaymentMethod Verify Checkout Session Payment Method

Verify that a checkout session and payment method belong to the same customer.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param checkoutSecureToken The checkout session's secure token
 @param paymentMethodId The payment method ID to verify
 @return ApiVerifyCheckoutSessionPaymentMethodRequest
*/
func (a *CheckoutAPIService) VerifyCheckoutSessionPaymentMethod(ctx context.Context, checkoutSecureToken string, paymentMethodId string) ApiVerifyCheckoutSessionPaymentMethodRequest {
	return ApiVerifyCheckoutSessionPaymentMethodRequest{
		ApiService: a,
		ctx: ctx,
...
// Execute executes the request
//  @return CheckoutSessionExternal
func (a *CheckoutAPIService) VerifyCheckoutSessionPaymentMethodExecute(r ApiVerifyCheckoutSessionPaymentMethodRequest) (*CheckoutSessionExternal, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CheckoutAPIService.VerifyCheckoutSessionPaymentMethod")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/checkout/sessions/{checkout_secure_token}/verify_payment_method/{payment_method_id}"
	localVarPath = strings.Replace(localVarPath, "{"+"checkout_secure_token"+"}", url.PathEscape(parameterValueToString(r.checkoutSecureToken, "checkoutSecureToken")), -1)
